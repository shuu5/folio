...
# --- spec_path 正規化 ---------------------------------------------------------
# FOLIO_SPEC_PATH (既定 "design-intent/spec/") を読み、 末尾 slash 1 個に正規化して
# stdout に返す。 `${:-}` は unset + empty 両対応 (二重 fallback 不要)。
folio_spec_path() {
  local p="${FOLIO_SPEC_PATH:-design-intent/spec/}"
  p="${p%/}/"
  printf '%s' "$p"
}
...
#   "$2"*    = 相対 path (例 "design-intent/spec/x.html") の prefix 一致
#   *"/$2"*  = "/spec_path/" を含む path、 特に絶対 path
#              (例 "/repo/design-intent/spec/x.html") に対応。 Claude Code は hook の
