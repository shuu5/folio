#!/usr/bin/env bash
# folio engine (folio-bxpm) — bootstrap extractor: srs-verification.html → folio-srs-verification.spec.yaml (one-shot)
#
# ★extract-verification-spec.sh の doc-type=spec 3例目への FORK (srs-verification self-host・8dkl flip)。 共有 core (lib/) 無改変。
# design-intent/spec/srs-verification.html を *read-only* で走査し、 spec-pack contract (folio-srs-verification.spec.yaml) の
...
#
# usage: extract-srs-verification-spec.sh [<srs-verification.html>] > <draft contract.yaml>   (LOG は stderr)
#        既定 <srs-verification.html> = <repo-root>/design-intent/spec/srs-verification.html

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
VERIF="${1:-$REPO_ROOT/design-intent/spec/srs-verification.html}"
[[ -f "$VERIF" ]] || { echo "extract-srs-verification-spec: srs-verification.html not found: $VERIF" >&2; exit 1; }
command -v perl >/dev/null || { echo "extract-srs-verification-spec: perl required" >&2; exit 1; }
...
my %TINT = (
  s0=>"info", s1=>"brand", s2=>"violet", s3=>"info", s4=>"warn",
);
my %KICK = (
  s0=>"§0 / 読み方", s1=>"§1 / 検証境界", s2=>"§2 / done-condition",
  s3=>"§3 / 被覆 RTM", s4=>"§4 / 参照",
);
# srs-verification.html の実 section id (full form)。 contract section.id は short prefix (s0..s4) を使う (TINT/KICK の key)。
my @SECORDER = qw(s0-reader-guide s1-boundary-model s2-done-condition-req
                  s3-coverage-rtm s4-references);
...
# ===== YAML 出力 =====
print "# folio engine (folio-bxpm) — spec-pack contract (instance#3 doc-type=spec / srs-verification self-host)\n";
print "# ★機械抽出 DRAFT (scripts/extract-srs-verification-spec.sh が design-intent/spec/srs-verification.html から起こした)。 人間レビュー前提。\n";
print "# doc_type = spec (folio SRS verification spec)。 EARS 章立て規範文 + 非終端 照会 (前方 references)。\n";
...
print "\n";
print "meta:\n";
print "  doc_id: FOLIO-SRS-VERIFICATION\n";
print "  doc_type: spec\n";
print "  title: ", ys("folio SRS verification spec"), "\n";
print "  eyebrow_left: ", ys("仕様 (Spec)"), "\n";
print "  eyebrow_right: ", ys("folio — 生成 SRS done-condition"), "\n";
print "  subtitle: ", ys($hmeta ne "" ? $hmeta : "生成 SRS プレゼンの done-condition (完成判定) gate の SSoT — floor + ceiling 二層"), "\n";
print "  version: ", ys($meta{version}), "\n";
print "  date: ", ys("2026-07-16"), "\n";
...
print "\n";
print "# 承認記録 (core 共用 approval-block)。 srs-verification.html は署名欄を持たないため doc lifecycle を忠実に再提示する (synthesized chrome)。\n";
print "approval:\n";
print "  - { role: ", ys("文書種別"), ", who: ", ys("spec (SRS verification)"), ", when: ", ys("v$meta{version}"), ", stamp: ", ys("active"), " }\n";
print "  - { role: ", ys("生成 (engine spec-pack)"), ", who: ", ys("folio design system"), ", when: ", ys("2026-07-16 生成"), ", stamp: ", ys("生成"), " }\n";
print "\n";
print "# ★非終端 照会の graph 接続: srs-verification → constitution (FOLIO-CONSTITUTION 終端) の前方 edge (role=implementation)。\n";
print "#   srs-verification は constitution P-x (P-13 検証&追跡 等) を前方照会し実装/検証する (folio:extends constitution)。\n";
print "graph:\n";
...
print STDERR "=== extract-srs-verification-spec LOG (no silent caps) ===\n";
print STDERR "$_\n" for @LOG;
