...
if [[ "$fail" -eq 0 ]]; then
  ...
  if [[ "${DATAMODEL_SKIP_RENDER:-0}" == "1" || "${SKIP_RENDER:-0}" == "1" ]]; then
    echo "  ※ render gate 未完 (F=見た目崩れ が未実行: DATAMODEL_SKIP_RENDER/SKIP_RENDER) — CI/uv 環境で render を回すまで floor は不完全。"
  fi
  echo "  ceiling = persona-walk-datamodel + fidelity-datamodel (agents/、 LLM review)。 floor 単独で GREEN を宣言しない。"
