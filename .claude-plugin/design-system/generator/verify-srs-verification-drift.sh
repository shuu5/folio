#!/usr/bin/env bash
# verify-srs-verification-drift.sh — in-scan generated-canonical drift gate (folio-lwhz F8 の srs-verification 版・folio-bxpm instance#3)
#
# design-intent/spec/srs-verification.html は inventory node (in-scan) だが、 folio validate の gate(m)
...
#     規約の非 vacuous 性は committed な test-adversarial-srs-verification-drift.sh が per-shape MK で pin する。
...
#
# usage: verify-srs-verification-drift.sh [--canon <path>]
#   --canon <path>  検査対象 canonical を上書き (既定 = <root>/design-intent/spec/srs-verification.html)。
...
fail() { echo "verify-srs-verification-drift: FAIL: $*" >&2; exit 1; }
...
if GIT_TOP="$(cd "$ROOT" && git rev-parse --show-toplevel 2>/dev/null)"; then
  [[ "$GIT_TOP" == "$ROOT" ]] || echo "verify-srs-verification-drift: warn: git-top($GIT_TOP) != script-relative($ROOT); script-relative を採用" >&2
fi

GEN="$ROOT/.claude-plugin/design-system/generator"
ASSEMBLE="$GEN/assemble-srs-verification.sh"
INJECT="$GEN/inject-prose.sh"
CONTRACT="$GEN/contract/folio-srs-verification.spec.yaml"
PROSE="$GEN/prose/folio-srs-verification.prose.yaml"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "verify-srs-verification-drift: 未知の引数 '$1'" >&2; exit 2 ;;
  esac
done
...
normalize() {
  perl -pe 's{生成: <b>\d{4}-\d{2}-\d{2} \d{2}:\d{2}</b>}{生成: <b>\@\@SRS-VERIFICATION-DRIFT-GATE-TS\@\@</b>}g'
}
...
"$SGEN/assemble-srs-verification.sh" "$SGEN/contract/folio-srs-verification.spec.yaml" "$STG/asm.html" >/dev/null 2>"$WORK/aerr" \
  || fail "assemble 失敗 (再生成不能): $(tr '\n' ' ' < "$WORK/aerr")"
"$SGEN/inject-prose.sh" "$SGEN/prose/folio-srs-verification.prose.yaml" "$STG/asm.html" "$STG/design-intent/spec/srs-verification.html" >/dev/null 2>"$WORK/ierr" \
...
FRESH="$STG/design-intent/spec/srs-verification.html"
...
fsz="$(wc -c < "$FRESH")"
# ★下限は srs-verification 実測 literal で pin (verification の 40000 を盲目 copy しない・folio-bxpm §8)。
#   現 fresh 実測 ≈ 130385 byte ゆえ 100000 で ~25% 収縮を捕捉 (byte-identity diff が主 teeth・本 floor は collapse 前検知)。
...
if ! diff -q "$WORK/canon.norm" "$WORK/fresh.norm" >/dev/null; then
  {
    echo "verify-srs-verification-drift: FAIL: design-intent/spec/srs-verification.html が fresh 再生成と drift (生成 timestamp 正規化後も byte 不一致):"
...
echo "verify-srs-verification-drift: PASS: design-intent/spec/srs-verification.html == fresh 再生成 (assemble-srs-verification + inject-prose + fix + build full pipeline・生成 timestamp 正規化後 byte 一致・物理指紋一致)"
