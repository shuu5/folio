...
act_scin="$(perl -CSD -0777 -ne 'if (/class="scol in">(.*?)<\/div>/s){ my $b=$1; while($b=~/<li>(.*?)<\/li>/gs){ my $it=$1; $it=~s/^<span class="b">[^<]*<\/span>//; print "$it\n" } }' "$STRIPPED")"
chk "body-prose: scope.in (scol in の li) == .scope.in (順序)" "$exp_scin" "$act_scin"
exp_scout="$(qesc '.scope.out[]')"
act_scout="$(perl -CSD -0777 -ne 'if (/class="scol out">(.*?)<\/div>/s){ my $b=$1; while($b=~/<li>(.*?)<\/li>/gs){ my $it=$1; $it=~s/^<span class="b">[^<]*<\/span>//; print "$it\n" } }' "$STRIPPED")"
