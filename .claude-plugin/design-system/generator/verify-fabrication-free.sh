...
#   ★アンカーは class でなく *data-acc-link* (set_eq §6 が検証済の attr)。 chk_empty は『取りこぼした要素=未検査=pass』ゆえ
#   class="dot ac" 固定だと大文字化 (CLASS=) で要素を隠して可視改竄を素通せた (admin 自力点検の case 兄弟)。 data-acc-link 起点なら
#   class 構文に非依存・data-acc-link 自体の大文字化は §6 set_eq が落とす。 [^>]* で末尾属性の順序に頑健。
acc_vis_bad="$(perl -CSD -0777 -ne 'while (/data-acc-link="[^"]*__([^"]*)"[^>]*>([^<]*)<\/span>/g){ push @b,"$1\x{2260}$2" if $1 ne $2; } END{ print join(" ",@b); }' "$BODY")"
chk_empty "within-doc: 受入ドット可視 == data-acc-link suffix (class 非依存)" "$acc_vis_bad"
