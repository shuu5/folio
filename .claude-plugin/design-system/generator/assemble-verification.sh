...
# ============================================================================
# ★ADR-0054 (flip 済 spec 提示層の標準形) lockstep 定数群 — verify-verification.sh と ★二重保守 (detect↔remediate parity)。
...
#   closed allowlist → 表示 class / 平易語の canonical 語幹。 verify-verification.sh と二重保守 = detect↔remediate parity。
...
#   ★集合の拡張には prose manifest / 本 allowlist / verify-verification.sh の同名配列の三点同時更新が必要 (fail-closed)。
declare -A PRIO_OK=( [must]=1 [should]=1 )
declare -A PRIO_LABEL_OK=( [must]="必須|必須・将来" [should]="推奨・現在" )
# ★静的 band (前方照会 / 用語集) の heading ★本文 (§番号を ★除いた 部分)。 verify-verification.sh の STATIC_HEADING_TAILS と
#   二重保守。 ★見出しは ★実在する照会種別のみを約束する (ADR-0054 §2.2 over-promise 禁止)。 verification の references は
#   原則 (P-x) / 決定記録 (ADR) の ★2 種のみ が実在する (rules の 3 種と異なる = 逐語流用でなく実測導出。 実測:
#   role=implementation 7 (constitution P-x) + role=rationale 23 (decisions/) = 30・REQ-VER 系 cross-ref は 0 件)。
#   ★結果として Cell R (relations = 同じ 2 種) と ★同一 逐語になる (偶然でなく照会種別集合が一致するため)。
...
#   ★class は付けない (normative/informative census を不変に保つ)。 verify-verification.sh の同名配列と二重保守。
PRESENTATION_WRAPPER_IDS=("forward-refs" "glossary-terms")
# ★機械層 fold / 要件 normative fold の平易ラベル (ADR-0054 §2.2・wave 恒常 2)。 verify-verification.sh と二重保守。
...
validate() {
  ...
  if [[ "$(q 'has("references")')" == "true" ]]; then
    ...
    #   verify-verification.sh の契約非依存 census floor (rf-gloss == 30) が封鎖する。
  ...
  #   verify-verification.sh の (iii) と ★二重保守 = detect↔remediate parity。
...
heading_secnum() { # $1 = heading
  ...
  [[ -n "$n" ]] || { echo "assemble-verification: ★heading が §N 形でない (節番号を導出できない・fail-closed): $1" >&2; exit 1; }
  printf '%s' "$n"
}
...
band_num() { # num tint kicker heading icon_inner
  ...
  [[ "$num" =~ ^[0-9]+$ ]] || { echo "assemble-verification: ★band_num に非数値の節番号 ('$num') が渡された (節番号を導出できない・fail-closed)" >&2; exit 1; }
  shift
  t="$(mktemp)"
  band "$1" "$2" "$3" "$4" > "$t"
  printf -v seq '%02d' "$CHAPN"      # 直前の band() が emit した連番 (core が進めた値)
  grep -qF "<span class=\"num\">$seq</span>" "$t" \
    || { rm -f "$t"; echo "assemble-verification: ★band の連番 .num ($seq) が emit に見当たらない (core band() の shape drift・fail-closed)" >&2; exit 1; }
...
emit_requirement_row() {
  ...
  if [[ -n "$prio" && "$prio" != "null" ]]; then
    [[ -v PRIO_OK[$prio] ]] || { echo "assemble-verification: ★到達不能: emit 時に未知 priority '$prio' (validate を擦り抜けた・fail-closed)" >&2; exit 1; }
...
emit_references() {
  ...
  q '.references[] | [.token, .doc, .role, (.title // "")] | @tsv' | while IFS= read -r line; do
    ...
    [[ -v ROLE_PLAIN[$role] ]] || { echo "assemble-verification: ★到達不能: emit 時に平易語 map 外の role '$role' (validate を擦り抜けた・fail-closed)" >&2; exit 1; }
