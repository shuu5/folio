...
# ★各 wrapper が包む章帯の tint (assemble-relations.sh build() の band_num 実引数 violet / brand と ★二重保守)。
#   containment の「開口隣接 literal pin」で band の ★同一性 (どの章の帯か) まで逐値要求するために持つ
#   (tint を持たないと「隣に *何らかの* band が在る」しか言えず、 別章 band の付け替えが素通る)。
#   ★契約章 7 本の tint 源は ★contract の sections[].tint ゆえ本配列は wrapper 2 章 ★専用 (両者の長さ一致は
...
#   生成物実測への after-the-fact 合わせ (恒真化) にならない。 ★契約章 7 本にも同じ (i)(ii)(iii) を課す
#   (2 id の手書き列挙に留めると同一 DOM shape を持つ残り 7 章が無防備 = partial-enumeration trap。 駆動は contract の section census)。
# ★per-shape mutation-kill (test-adversarial-relations.sh・★全 arm が 1 つ以上の実弾で anchor される):
...
#   hollow 化 (2 id 列挙では届かない残り 7 章) / CT9 chapbody 空化 + payload を sibling へ (1 段内側の帰属) /
...
#   CT41 ★attribute-name state への quote 混入 (幽霊 marker の第 2 vector・CT36 は値 state のみ塞いでいた) /
#   CT42 ★タグ境界 quote 走査の state 非同型による ★実在する閉じタグの隠蔽 (`<div class="ref-grid" x=1 "></section>">`
#   の ★1 行で containment 9 arm を全 bypass した errata 3 の封鎖・旧「開示済み残差 ①」) /
#   CT43 / CT44 / CT45 ★block wrapper の中身の逃がし (rq-list ごと fold へ + 空 filler / tbl-wrap ごと fold へ +
#   空 filler / tbl-wrap 据置で中の table だけ fold へ = ★errata-1・admin gate round-1 blocking。 いずれも
#   封鎖前は rc=0 / 220 OK / 0 FAIL で ★clean と完全一致 だった = 規範要件や表が ★既定折り畳みへ silent 退避) /
#   CT46 / CT47 ★同章内の 器 → 器 payload 再配分 (器 3 本の s1 / 器 5 本の s4 で 1 本目へ寄せ 2 本目を空の器に =
#   ★errata-2・admin gate round-2 blocking。 封鎖前は rc=0 / 228 OK / 0 FAIL で ★clean と完全一致 = 章合計 pin の
#   構造的死角。 可視には §1.2 の表が空になり §1.1 の見出し下へ誤帰属する破損が起きる) /
#   CT32 ★void 要素 2 個による ★深さ 1 計数の
#   打ち切り (章本文 relocation のマスク・封鎖前は 3 レベル束縛が ★全て素通る fail-open を実測) /
#   CT33 ★foreign root を depth-1 filler にした完全子数マスク / CT34 ★foreign 内の any-other-end-tag
#   (`<svg></section></svg>` = 実 parser は外側 section を閉じる) / CT35 ★foreign 内の breakout 開始タグ
#   (svg 内 div.ref-grid = 実 DOM では器が 2 個) / CT37 ★svg 包みによる人間層 block の章外 relocation
#   (CT33/34/35/37 は「foreign subtree を丸ごと読み飛ばす」旧実装が containment 9 arm を ★1 行で全 bypass
#   させた errata の per-shape 封鎖) / CT36 ★unquoted 属性値への quote 混入 (実 DOM に無い ★幽霊 marker を
#   属性逐値一致が数える形・cont / docct / cbadj / pay / tint に横断で効く primitive)
#   — parser-differential 系は ★合計 13 vector で、 ★いずれも 封鎖 ★前 は rc=0 / 0 FAIL で
#   素通った 実測 shape である (CT1-CT32 は範型 = verification arm でも同じく素通り、 CT33-CT45 は本 pack の
#   errata として ★relations 側で実測・範型は未封鎖のまま = 上の ★false record の是正 節を参照)。
#   ★CT43-45 は parser-differential ではなく ★「器の中身を束縛していない」構造欠落 (admin gate round-1 blocking)。
...
#   per-shape MK を持てない。 これらは test-adversarial-relations.sh の ★MECHPIN 節 (tracked 静的 pin) が
...
#   ★errata-1 (admin gate round-1 blocking) の記録: 旧版は前提 (0) = 「depth-1 marker が文書 census か
#   器中身 pin のどちらかで個数拘束される」を ★欠いたまま クラス閉塞を宣言しており、 wrapper class が
#   census を持たない 2 type (table / requirements) で ★3 shape が全 arm 素通り だった (実測)。 現在は
#   器中身の占有 pin + 器自身の文書 census を新設し、 未登録 type を fail-closed にして閉じている。
...
#   ★err で拒否する (errata: 旧実装の「subtree 丸ごと読み飛ばし」は containment 9 arm を ★1 行で全 bypass
#   させた — 実測 4 vector = CT33 / CT34 / CT35 / CT37)、 タグ境界は 引用符を ★HTML5 の attribute-name /
#   attribute-value state に ★同型に (quote が値を開くのは `=` の直後だけ・errata 3 = 旧 残差 ①・CT42) 認識して 決定し、
#   要素名は ★ハイフン込みで 1 語 として取る。 占有数も 生テキスト grep でなく ★tokenize 済タグの属性 逐値一致
#   で数える (属性値に marker 文字列を紛れ込ませた計数水増しの封鎖)。 ★宣言の射程 (★errata で 2 度改訂):
#   属性の走査は タグ名直後から ★attribute-name state → attribute-value state を ★順に消費する 逐次形であり、
#   ★名前 state は `"` / `\x27` / `<` を ★名前文字として取り込む (HTML5 同型)。 これで 「実 DOM に無い marker を
#   probe が拾う」★幽霊 marker クラス は ★値 state (unquoted 値の char class = `[^\s>]+`・CT36) と
#   ★名前 state (`<div a""class="ref-grid">`・CT41) の ★両方 で塞がった。 ★errata の記録: 旧コメントは
#   CT36 の封鎖だけを見て「逆向きは 属性 tokenize の同型化で ★消えている」と ★断定していた が、 名前 state は
#   開いたまま で 1 置換の幽霊 marker が ★実測 verified だった (false record)。 ★文字参照 も ★解決する
#   (数値 + 名前付き 6 種・表に無い名前付きは err = fail-closed)。 旧コメントは 未解決を「probe が数え落とす
#   向き = ★常に fail-closed 側」と一般化していたが これも ★誤り で、 ★上限型 == arm (docct) では
#   数え落とし = ★silent PASS だった (実測 verified・CT40)。 ★undercount の向きは arm の期待値の型に依存する
#   — 「落とす側だから安全」と ★一般化してはならない (本 file 内の他の残差記述にも同じ規律を適用する)。
...
#   ★★旧 開示済み残差 ① → ★封鎖済 (errata 3・本 cell)。 ★何が空いていたかを 具体的に残す (「直した」で
#   括らない): 旧実装のタグ境界走査は quote 認識が HTML5 の ★attribute-value state 限定ではなく、 `=` を
#   伴わない `"` / `'` でも quote 開始として扱っていた。 実 parser では before/after-attribute-name state の
#   `"` は ★属性名 であり、 その後の最初の `>` がタグを終端する。 ゆえに ★genuine タグへ ` x=1 "></section>"`
#   を 1 個足す だけで — 実測形は `<div class="ref-grid" x=1 "></section>">` — ★実 DOM では そこで章が閉じ
#   chapbody / payload (chip 13) が章の ★外 へ出る のに、 旧走査は `</section>` を quote 内と見なして region を
#   元の閉じタグまで伸ばし、 adj/band/cb/kids/bkids/cbkids/cbadj/cont/pay/docct が ★全て期待値どおり になった
#   (★封鎖前 実測: rc=0 / 220 OK / 0 FAIL = ★1 行 bypass。 実 DOM 側は Python html.parser で `S div.ref-grid`
#   の直後に `E section` が来ることを確認。 上流 make_body の raw-lt-in-tag も タグ先頭〜最初の `>` に生 `<` が
#   無いため掛からない)。 ★現在は タグ境界を attribute-name / value の 6 state へ同型化し、 引用値直後の
#   非 genuine 文字を err (`bogus-after-quoted-value-*`) へ倒して封鎖している (per-shape MK = CT42)。
#   ★依存の所在: この走査は ★範型 verify-verification.sh と 同一実装だった ため ★範型には同じ fail-open が残る
#   (base-pinned の継承・実測)。 本 pack だけ強化したので ★arm drift が生じる — 是正 (範型 + spec / srs への
#   同期) は本 cell の fence で触れないため ★admin への申し送り事項 とする (下記 ★受け皿)。
#   ★★false record の是正 (宣言 == 実能力): 「本 probe は範型と ★byte 同一」と書いてはならない — ★もはや
#   同一部分は無い (タグ境界走査も errata 3 で分岐した)。 本 pack は範型に ★無い fail-closed を 7 つ持つ:
#   (g) タグ境界の attribute state 同型化 + 引用値直後の fail-closed (CT42) (a) count_kids の打ち切りを
#   err (`unclosed-child-*`) へ倒す (CT32) (b) foreign content の stack 追跡 + depth-1 計上
#   (`foreign-breakout-*` / `foreign-at-depth1-*`・CT33-35/37) (c) $attr の unquoted 値 char class 同型化 (CT36)
#   (d) $attr の ★逐次消費化 = attribute-name state 同型 (幽霊 marker の第 2 vector・CT41)
#   (e) 属性値の ★文字参照解決 + 表外参照の err (`unresolved-charref-*`・CT40)
#   (f) chapbody 直下を ★数 でなく ★marker 列 で束縛 (filler 置換 relocation・CT38 / CT39)。
#   ★裏返しの事実 も開示する: 範型 verify-verification.sh (および同型移植 ★前 の spec / srs pack) には これらが
#   ★無い ので、 CT32 / CT33-37 / CT38-42 が突く fail-open は ★sibling 側に残っている (範型の該当 probe を同一入力へ
#   通すと err='' で素通ることを実測)。 範型のコメントは今も「この範囲でクラス閉塞が成立する」と書いており、
#   ★実能力より強い宣言が sibling に残る。 他 pack の verify script は本 cell の fence で ★1 byte も触れない
#   ため ここでは是正できない — ★受け皿 = admin (Leg Z = folio-3zr4.4 か新規 follow-up)。 ★Leg A / Leg C の
#   同型移植は 範型でなく ★本 file の probe (上記 7 fail-closed 込み) を移植元に取ること。
#   ★開示済み残差 ② — foreign content (svg / math) の ★中 の非同型 (errata で ★大半を封鎖 した後の残り)。
#   現実装は subtree を ★本体と同じタグ走査で 追い、 breakout 開始タグ (%FOREIGN_BREAKOUT) と stack に無い
#   終了タグ を err にする ので、 実測 verified だった 3 vector (svg 内 `</section>` / svg 内 div.ref-grid /
#   svg 包みの章外 relocation) と、 root 計上が ★逆に開けてしまう depth-1 filler マスク (`foreign-at-depth1-*`
#   で対に封鎖) は 塞がった。 ★残るのは 「実 parser が foreign を抜ける 規則のうち 本実装が
#   モデルしていない もの」 = (i) SVG script / MathML annotation-xml の integration point (中身が HTML
#   名前空間に戻る文脈) (ii) 文字参照 (iii) font の属性条件 (本実装は ★属性を見ずに breakout 扱い =
#   fail-closed 側へ寄せた過剰)。 ★genuine 生成物の svg subtree は path / circle の自己閉じのみ (実測) ゆえ
#   これらは現時点で ★到達しない が、 「閉じた」ではなく「この範囲で閉じた」と読むこと。
...
#   ★同 (LOW-20/21 → 封鎖済): count_kids は ★同名深さ計数 ゆえ void 要素が depth-1 に現れると計数がそこで
#   打ち切られる。 これを ★silent に打ち切ると「章本文 block を 1 個 chapbody の外へ出し、 抜けた分の位置に
#   void を 1 個挿す」だけで 3 レベルの完全子数束縛が ★全て素通る (実弾 rc=0 / 0 FAIL を再現した)。 現在は
#   打ち切りを ★err (`unclosed-child-<要素名>`) へ倒す fail-closed で封鎖している (per-shape MK = CT32)。
...
#   lib/common.sh の band() / band_end() が emit する形であり、 pack-local な assemble-relations.sh ではない。
  ...
  perl -CSD -0777 -ne '
    ...
    while ($i < $n) {
      ...
      if ($win =~ m{^<(/?)([a-zA-Z][a-zA-Z0-9-]*)}) {
        ...
        #   ★1 行 bypass・実測 verified = 封鎖前 rc=0 / 220 OK / 0 FAIL)。 実 parser では before/after-attribute-name
    ...
    #   倒れる (実測 verified: `</body>` 直前へ `<div class="ref&#45;grid"></div>` を足すと 実 DOM の
    ...
    for my $line (split /\n/, ($ENV{PROBE_SPEC} // "")) {
      ...
      if ($found == 1) {
        ...
        if ($r0 >= 0 && $r0 <= $r1) {
          ...
          #   block 退避が ★どのレベルの子数も動かさず 素通る (実測 verified: rc=0 / 220 OK / 0 FAIL。
        ...
        if ($cbk >= 0) {
          ...
          if ($q0 >= 0 && $q0 <= $q1) {
            for my $ci ($kids_idx->($q0, $q1)) {
              ...
              #   素通る (実測 shape D/D2: rc=0 / 228 OK / 0 FAIL = clean と 1 arm も違わない)。 列にすると
...
#   1 段内側 (iii) は ★全 9 章が持つ — wrapper 2 章は payload container (ref-grid / glossary-term-table) と
#   payload 全数、 契約章 7 本は ★章要旨 callout (assembler が chapbody 開口直後に必ず emit する部品) を器に取る。
...
[[ "${#_SEC_TINTS[@]}" -eq "${#_SEC_ANCHORS[@]}" ]] || { echo "verify-relations: ★contract sections[].tint の件数が anchor と不一致 (${#_SEC_TINTS[@]} != ${#_SEC_ANCHORS[@]}・tint 逐値 pin の trust anchor 欠落・fail-closed)" >&2; exit 1; }
...
#   源 = assemble-relations.sh の emit_blocks 分岐 (1 block = ★1 つの depth-1 要素) ゆえ ★二重保守 (lockstep 相手の
...
declare -A CHAPBODY_KID_MARK=(
  ...
  [subhead]="div:spec-subhead"    [subsubhead]="div:spec-subsubhead"
  [requirements]="div:rq-list"    [ref-primary]="ul:spec-ref-primary"
)
# ★契約章 7 本の 1 段内側 (iii) は ★章要旨 callout を器に取る。 assemble-relations.sh の emit_section が
...
for _k in "${!_SEC_ANCHORS[@]}"; do
  [[ -n "${_SEC_TINTS[$_k]}" && "${_SEC_TINTS[$_k]}" != "null" ]] || { echo "verify-relations: ★contract sections[$_k].tint が空 (tint 逐値 pin の trust anchor 欠落・fail-closed)" >&2; exit 1; }
  ...
  #   machine_blocks があれば fold 1 (assemble-relations.sh の emit_blocks は 1 block = ★1 つの depth-1 要素、
  ...
  [[ "$_nb" =~ ^[0-9]+$ && "$_nm" =~ ^[0-9]+$ ]] || { echo "verify-relations: ★sections[$_k] の blocks / machine_blocks 件数を導出できない (cbkids 期待値の trust anchor 欠落・fail-closed)" >&2; exit 1; }
  _sig="div:section-essence-callout"; _cnt=0
  while IFS= read -r _bt; do
    [[ -n "$_bt" ]] || continue
    _bm="${CHAPBODY_KID_MARK[$_bt]:-}"
    [[ -n "$_bm" ]] || { echo "verify-relations: ★sections[$_k] の block type '$_bt' が CHAPBODY_KID_MARK に無い (chapbody 完全子列の導出不能・silent skip 禁止・fail-closed)" >&2; exit 1; }
  ...
  [[ "$_cnt" -eq "$_nb" ]] || { echo "verify-relations: ★sections[$_k] の block type 列 ($_cnt 件) が blocks 件数 ($_nb) と不一致 (chapbody 完全子列の trust anchor 破れ・fail-closed)" >&2; exit 1; }
...
#   (実測 3 shape・rc=0 / OK 数 baseline 完全一致 = arm skip ではなく ★全素通り)。
# ★CENSUS_BACKED_MARK = 上の「1. 行数」節が ★文書全体で 個数を数えている data-component token の逐値集合
#   (assemble-relations.sh / 同節と ★三点二重保守)。 depth-1 marker が この集合に ★属さない block type は
...
declare -A CENSUS_BACKED_MARK=(
  ...
  [spec-subhead]=1 [spec-subsubhead]=1 [spec-ref-primary]=1
)
# ★block wrapper → (器要素名|器属性|器値|payload 要素名|payload 属性|payload 値)。 源 = assemble-relations.sh の
...
[[ "${#BLOCK_WRAPPER_TYPES[@]}" -eq "${#BLOCK_WRAPPER_SPEC[@]}" ]] \
  || { echo "verify-relations: ★BLOCK_WRAPPER_TYPES と BLOCK_WRAPPER_SPEC の件数不一致 (lockstep 破れ・fail-closed)" >&2; exit 1; }
for _wt in "${BLOCK_WRAPPER_TYPES[@]}"; do
  [[ -v BLOCK_WRAPPER_SPEC[$_wt] && -v BLOCK_WRAPPER_CLBL[$_wt] && -v BLOCK_WRAPPER_PLBL[$_wt] ]] \
    || { echo "verify-relations: ★BLOCK_WRAPPER_* に型 '$_wt' の定義が欠けている (fail-closed)" >&2; exit 1; }
done
...
[[ "${#_BT_ALL[@]}" -gt 0 ]] \
  || { echo "verify-relations: ★contract の block type 列挙が空 (partial-enum guard が恒真化する・fail-closed)" >&2; exit 1; }
[[ "$_BT_N" =~ ^[0-9]+$ && "${#_BT_ALL[@]}" -eq "$_BT_N" ]] \
  || { echo "verify-relations: ★block type 列挙の件数不一致 (${#_BT_ALL[@]} != ${_BT_N}・guard 入力の trust anchor 破れ・fail-closed)" >&2; exit 1; }
for _bt in "${_BT_ALL[@]}"; do
  [[ -n "$_bt" ]] || { echo "verify-relations: ★空の block type が列挙に混入 (fail-closed)" >&2; exit 1; }
  _bm="${CHAPBODY_KID_MARK[$_bt]:-}"; _bmk="${_bm#*:}"
  [[ -n "$_bm" ]] || { echo "verify-relations: ★block type '$_bt' が CHAPBODY_KID_MARK に無い (fail-closed)" >&2; exit 1; }
  [[ -v CENSUS_BACKED_MARK[$_bmk] || -v BLOCK_WRAPPER_SPEC[$_bt] ]] \
    || { echo "verify-relations: ★block type '$_bt' の depth-1 marker '$_bmk' は文書 census を持たず BLOCK_WRAPPER_SPEC 未登録 (器の中身が無検査・fail-closed)" >&2; exit 1; }
done
...
for _wt in "${BLOCK_WRAPPER_TYPES[@]}"; do
  ...
  [[ "$_wtot" =~ ^[0-9]+$ ]] || { echo "verify-relations: ★block type '$_wt' の総数を導出できない (fail-closed)" >&2; exit 1; }
  ...
  for _k in "${!_SEC_ANCHORS[@]}"; do
    ...
    [[ "$_wn" =~ ^[0-9]+$ ]] || { echo "verify-relations: ★sections[$_k] の '$_wt' 件数を導出できない (fail-closed)" >&2; exit 1; }
    [[ "$_wn" -gt 0 ]] || continue
    ...
    if [[ "$_wt" == "requirements" ]]; then
      while IFS= read -r _il; do
        [[ -n "$_il" ]] || continue
        [[ "$_il" =~ ^[0-9]+$ && "$_il" -gt 0 ]] || { echo "verify-relations: ★sections[$_k] の requirements block の ids 数を導出できない/0: '$_il' (0/0 恒真回避の前提破れ・fail-closed)" >&2; exit 1; }
        _wp+="${_wp:+,}$_il"
    ...
    [[ "$_wpcnt" -eq "$_wn" ]] || { echo "verify-relations: ★sections[$_k] の '$_wt' 期待列 ($_wpcnt 要素) が器数 ($_wn) と不一致 (器ごと期待値の trust anchor 破れ・fail-closed)" >&2; exit 1; }
  ...
  [[ "$_wtot" -eq 0 || "$_wrows" -gt 0 ]] \
    || { echo "verify-relations: ★block type '$_wt' は契約に $_wtot 件在るのに章 row が 0 本 (駆動表の空振り・fail-closed)" >&2; exit 1; }
done
_PROBE_SPEC=""
for _k in "${!_CT_ID[@]}"; do
...
[[ "${#PRESENTATION_WRAPPER_IDS[@]}" -eq "${#PRESENTATION_WRAPPER_BAND_TINTS[@]}" ]] \
  || { echo "verify-relations: ★PRESENTATION_WRAPPER_IDS と PRESENTATION_WRAPPER_BAND_TINTS の長さ不一致 (lockstep 破れ・fail-closed)" >&2; exit 1; }
...
for _k in "${!_CT_ID[@]}"; do
  ...
  #   動かさず 素通る (実測 verified: rc=0 / 220 OK / 0 FAIL・人間層見出しが ★既定非表示の <details> へ
  ...
  #   (β-1) ★marker 自身が文書 census を持つ type (spec-subhead / spec-note / spec-prose …) は、 空 filler を
  #   置くと ★文書全体の component census (例 spec-subhead == Σ subhead blocks) が 2 件を数えて loud に落ちる
  #   = ★列 pin と census の ★対 で閉じる。 (β-2) ★marker が wrapper class で 文書 census を持たない type
  #   ([table]=div.tbl-wrap / [requirements]=div.rq-list) は ★census が存在しない ため、 旧版は
  #   「列 pin と census の対で閉じる」と ★一律に 宣言していたが 実際には ★全 arm 素通り だった
  #   (実測 3 shape・rc=0 / 220 OK / 0 FAIL = clean と完全一致: rq-list ごと fold へ + 空 filler /
  ...
  #   同型の器が ★2 本以上 ある章 (現契約では s1 の 3 本 / s4 の 5 本) で「1 本目へ寄せて 2 本目を空の器として
  #   残す」形が ★全 arm 素通り した (実測 2 instance: rc=0 / 228 OK / 0 FAIL = ★clean と 1 arm も違わない。
  #   可視には §1.2 の表が空になり §1.1 の見出し下へ誤帰属する破損が起きる)。 ★現在は期待値を
  ...
  #   ★器 1 本の章は 合計 == 器単位 ゆえ本 shape に構成上 免疫 (撃つ実弾が作れない) — 実弾は s1 / s4 に置く。
  #   ★この範囲では クラス閉塞が成立する — ただし成立は ★4 つの前提に
  #   依存する と ★明示 する (宣言 == 実能力): (0) 深さ 1 marker が ★文書 census か ★器ごとの個数列 pin の
  #   どちらかで拘束される こと (上の β-2・errata-1 で新設 / errata-2 で ★章合計から器単位へ是正・
  ...
  #   attribute state 同型化 + 引用値直後の fail-closed で封鎖済 (CT42・上の ★旧 残差 ① 節)。 ★範型側は未封鎖
  ...
  #   源に持ち、 その値の SSOT は ★assemble-relations.sh の emit_blocks 分岐 である (pack-local・二重保守)。
