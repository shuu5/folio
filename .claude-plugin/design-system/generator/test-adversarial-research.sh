...
perl -0777 -i -pe 's#(<a data-component="cross-doc-leads-chip" href="[^"]*" data-ap-id="AP1".*?</a>)#$1$1#s' "$TMP/r18.html"
...
# === ★folio-c5r.9 cross-doc href 遷移先 fidelity 敵対 (Fork B coarse = ADR #decision 着地・href の # と衝突するため {} デリミタ)。 ===
# RH1. cross-doc href の anchor swap (#decision→#FAKE・1 本) → 全 href==#decision 整合で FAIL
cp "$TMP/base-filled.html" "$TMP/rh1.html"
perl -0777 -i -pe 's{(href="clinic-double-booking.adr.html)#decision(")}{${1}#FAKE${2}}' "$TMP/rh1.html"
expect_verify_fail_filled "RH1 ★cross-doc href anchor swap (#decision→#FAKE) を href 整合で捕捉" "$BASE_PROSE" "$BASE" "$TMP/rh1.html"
# RH2. cross-doc href の filename swap (外部 host・1 本) → 全 href==#decision 整合で FAIL
cp "$TMP/base-filled.html" "$TMP/rh2.html"
perl -0777 -i -pe 's{href="clinic-double-booking.adr.html#decision"}{href="https://evil.example#decision"}' "$TMP/rh2.html"
expect_verify_fail_filled "RH2 ★cross-doc href filename swap (外部 host) を href 整合で捕捉" "$BASE_PROSE" "$BASE" "$TMP/rh2.html"
...
if repro_pins "$VER" research "$BASE" "$BASE_PROSE" "$ASM" "$INJ" --filled "$BASE_PROSE"; then ok "repro-build conformance (a-d) 全 pass"; else ng "repro-build conformance (a-d) 逸脱"; fi
echo
