#!/usr/bin/env bash
# folio engine (folio-bxpm) — spec-pack 決定的 assembler の FORK (doc-type=spec / srs-verification self-host)
#
# ★assemble-verification.sh の FORK (doc-type=spec 3例目・srs-verification 8dkl flip)。 共有 core (lib/) 無改変。 count 定数
#   (RICH_FIELD_MIN / *_TYPE_MINS / MACHINE_*) を srs-verification.html 実測へ差替えた以外は assemble-verification.sh と同型
#   (demoted machine block type も継承・srs 契約には demoted は不在だが allowlist は保持)。
...
# ★検査対象の実測下限 (現契約 = 46 値)。 rich_field_values の query drift / 抽出規約の破綻で ★検査対象そのものが
...
#   (= 被覆の喪失) だけである。
# ★instance 固有値の扱い (汎化 fence): 本 file は srs-verification 専用 fork (validate() の doc_type==spec 束縛を参照)
#   ゆえ、 本定数は ★その fork の実測値。 同型を他 pack へ波及させる際は ★機構 (被覆量 assert) をそのまま持ち、
#   値だけ各 pack の実測へ差し替えること (機構を落として値だけ移すと恒真 PASS が復活する)。
# ★本 file は srs-verification 専用 fork (folio-bxpm)。 下記定数は srs-verification.html 実測 (verification の 337 を盲目 copy せず差替)。
RICH_FIELD_MIN=46
# ★型別の実測下限 (folio-eccf S5・RICH_FIELD_MIN の count-only 残余の解消)。 "min:::label:::query" の 3 つ組。
# ★なぜ総数下限だけでは足りないか (総数は ★型間の相殺を見逃す):
#   RICH_FIELD_MIN は「総数が減ったら FAIL」しか言えない。 契約が正当に育って table-rows が 25→30 になった後に
#   sect-essence (5 値) の query が drift すると 総数は 51 ≥ 46 ゆえ ★緑のまま 5 値が無検査になる。
#   すなわち ★被覆の喪失が契約の成長に紛れて隠れる。 ゆえ ★型ごとに下限を持ち、 型単位の減少を個別に撃つ。
# ★実測 (srs-verification 契約): 5 + 0 + 1 + 5 + 25 + 0 + 5 + 5 = 46 = RICH_FIELD_MIN (総数と内訳は同じ集合の別表現)。
...
RICH_FIELD_TYPE_MINS=(
  '5:::sect-essence:::.sections[].essence'
  '0:::subhead-essence:::[.sections[].blocks[]? | select(.type=="subhead") | .essence] | .[]'
  '1:::table-caption:::[.sections[].blocks[]? | select(.type=="table") | (.caption // "")] | .[]'
  '5:::table-headers:::[.sections[].blocks[]? | select(.type=="table") | .headers[]] | .[]'
  '25:::table-rows:::[.sections[].blocks[]? | select(.type=="table") | .rows[][]] | .[]'
  '0:::mermaid-caption:::[.sections[].blocks[]? | select(.type=="mermaid") | (.caption // "")] | .[]'
  '5:::req-essence:::.requirements[].essence'
  '5:::req-statement:::.requirements[].statement'
)
...
# ★機械層 field の実測下限 (現契約 = 14 値)。 人間層 RICH_FIELD_MIN=46 とは ★別物・★merge 禁止
#   (合算すると 46 の pin が壊れ、 かつ層別 allowlist の適用先が混ざる)。 意味論は RICH_FIELD_MIN と同じ「下限」で、
#   ★減少方向 (被覆の喪失) だけを塞ぐ。
MACHINE_FIELD_MIN=14
# ★機械層の ★型別 (= 抽出分岐別) 実測下限。 人間層 RICH_FIELD_TYPE_MINS と ★同型の機構・★別の定数束 (merge 禁止)。
# ★なぜ総数下限 (14) だけでは足りないか — 同一 diff の S5 が人間層で ★既に証明した穴と ★同型ゆえ:
#   MACHINE_FIELD_MIN=14 は ★たまたま現総数と一致している ので、 今日は任意分岐の drift が総数減で捕捉される。
#   しかし契約が正当に育って block 分岐が 3 値増えた瞬間 (総数 17)、 machine_preamble 分岐の query が
#   drift しても 11+3 = 14 ≥ 14 ゆえ ★総数 assert は緑のまま で、 preamble 3 値の RAW emit が ★無検査になる。
#   = 被覆の喪失が契約の成長に紛れて隠れる (S5 の 46/51 と厳密に同じ機序)。 ゆえ ★分岐ごとに下限を持つ。
# ★実測 (srs-verification 契約): 3 + 0 + 3 + 8 = 14 = MACHINE_FIELD_MIN (総数と内訳は同じ集合の別表現)。
...
MACHINE_FIELD_TYPE_MINS=(
  ...
  '3:::block-html:::.sections[].machine_blocks[]?|select(.type != "list")|.html'
  '8:::block-items:::.sections[].machine_blocks[]?|select(.type == "list")|.items[]'
)
...
validate_rich_inline() {
  ...
  if ! rich_field_values > "$vals"; then
    rm -f "$vals"
    echo "assemble-srs-verification: ★rich field の抽出に失敗 (yq 非 0 exit・検査不能ゆえ fail-closed)" >&2
    return 1
  fi
  ...
  if [[ "$n" -lt "$RICH_FIELD_MIN" ]]; then
    rm -f "$vals"
    echo "assemble-srs-verification: ★rich field の検査対象が $n 件 (期待下限 $RICH_FIELD_MIN)。 rich_field_values の" >&2
  ...
  for spec in "${RICH_FIELD_TYPE_MINS[@]}"; do
    ...
    if ! tn="$(q "[ $tquery ] | .[]" 2>/dev/null | wc -l | tr -d ' ')"; then
      rm -f "$vals"
      echo "assemble-srs-verification: ★rich field 型別 '$tlabel' の抽出に失敗 (検査不能ゆえ fail-closed)" >&2
      return 1
    fi
    sum=$((sum + tn))
    if [[ "$tn" -lt "$tmin" ]]; then
      rm -f "$vals"
      echo "assemble-srs-verification: ★rich field 型別 '$tlabel' が $tn 件 (期待下限 $tmin)。 型単位で被覆が痩せている" >&2
  ...
  if [[ "$sum" -ne "$n" ]]; then
    rm -f "$vals"
    echo "assemble-srs-verification: ★rich field の型別 総和 $sum が総行数 $n と不一致 (RICH_FIELD_TYPE_MINS と" >&2
  ...
  [[ $rc -eq 0 ]] || { echo "assemble-srs-verification: ★rich field の tokenize に失敗 (perl 非 0 exit・検査不能ゆえ fail-closed)" >&2; return 1; }
  [[ -z "$bad" ]] && return 0
  echo "assemble-srs-verification: ★人間層 rich field に allowlist 外の markup: $bad" >&2
...
validate_machine_inline() {
  ...
  if ! machine_field_values > "$vals"; then
    rm -f "$vals"
    echo "assemble-srs-verification: ★機械層 field の抽出に失敗 (yq 非 0 exit・検査不能ゆえ fail-closed)" >&2
    return 1
  fi
  ...
  if [[ "$n" -lt "$MACHINE_FIELD_MIN" ]]; then
    rm -f "$vals"
    echo "assemble-srs-verification: ★機械層 field の検査対象が $n 件 (期待下限 $MACHINE_FIELD_MIN)。 machine_field_values の" >&2
  ...
  for mspec in "${MACHINE_FIELD_TYPE_MINS[@]}"; do
    ...
    if ! tn="$(q "[ $tquery ] | .[]" 2>/dev/null | wc -l | tr -d ' ')"; then
      rm -f "$vals"
      echo "assemble-srs-verification: ★機械層 field 分岐 '$tlabel' の抽出に失敗 (検査不能ゆえ fail-closed)" >&2
      return 1
    fi
    msum=$((msum + tn))
    if [[ "$tn" -lt "$tmin" ]]; then
      rm -f "$vals"
      echo "assemble-srs-verification: ★機械層 field 分岐 '$tlabel' が $tn 件 (期待下限 $tmin)。 分岐単位で被覆が痩せている" >&2
  ...
  if [[ "$msum" -ne "$n" ]]; then
    rm -f "$vals"
    echo "assemble-srs-verification: ★機械層 field の分岐別 総和 $msum が総行数 $n と不一致 (MACHINE_FIELD_TYPE_MINS と" >&2
  ...
  [[ $rc -eq 0 ]] || { echo "assemble-srs-verification: ★機械層 field の tokenize に失敗 (perl 非 0 exit・検査不能ゆえ fail-closed)" >&2; return 1; }
  [[ -z "$bad" ]] && return 0
  echo "assemble-srs-verification: ★機械層 field に allowlist 外の markup: $bad" >&2
...
validate() {
  ...
  [[ "$(q '.meta.doc_type')" == "spec" ]] || { echo "assemble-srs-verification: ★meta.doc_type は spec 必須 (本 fork は verification 専用・doc_type flip で gate bypass 不可)" >&2; errs=1; }
  # 要件 id 一意性
...
emit_pack_head_meta() {
  ...
  [[ "$stake_type" == "!!seq" || "$stake_type" == "!!null" ]] \
    || { echo "assemble-srs-verification: ★meta.stakeholders は array 必須 (実際: $stake_type)。 scalar は CORE の JSON-LD folio:stakeholders を canonical の array から string へ型退行させる (jsonld-lint / inventory / fedges のいずれも捕捉しない fail-open)" >&2; exit 1; }
...
emit_head() {
  ...
  printf '<meta name="generator" content="folio spec-pack assembler — srs-verification fork (folio-bxpm) — deterministic structure, prose slots unfilled">\n'
...
emit_subhead() {
  ...
  [[ -n "$anchor" && "$anchor" != "null" ]] || { echo "assemble-srs-verification: ★subhead (section[$1] block[$2]) の anchor が空 (fine section anchor 宛て inbound の解決先を失う・fail-closed)" >&2; exit 1; }
...
emit_requirement_row() {
  ...
  [[ -n "$anchor" && "$anchor" != "null" ]] || { echo "assemble-srs-verification: ★要件 $id の anchor (navigable id) が空 (corpus inbound の解決先を失う・fail-closed)" >&2; exit 1; }
...
emit_machine_block() { # $1 = block への yq path (e.g. ".machine_preamble[0]" / ".sections[$si].machine_blocks[$bi]")
  ...
  case "$mt" in
    ...
    *) echo "assemble-srs-verification: ★到達不能: emit 時に未対応 machine block type '$mt' ($base・validate を擦り抜けた・fail-closed)" >&2; exit 1 ;;
  esac
}
...
emit_footer() {
  core_emit_footer '<span>folio design system</span><span>spec-pack (srs-verification fork)</span><span>folio engine</span><span>EARS 章立て + 非終端 照会 + 機械層 fold</span>'
}
