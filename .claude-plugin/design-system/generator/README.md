...
- **次の rule-of-three watch**: 可視 echo の micro-pattern (block 抽出 / 厳密一致 / nested-reject / count anchor) は research・ADR で多数反復するが、
  binding 変動が高いため **抽出は強制せず** 別 doc-type の 4 例目出現時に再評価する (B3 rule-of-three 監視の継続)。
- **非破壊**: 生成 artifact 全 8 種 byte 不変 (footer timestamp 除く・assemble は無改変ゆえ verify 側のみの変更)・昇格でスケルトン非弱化。
  敵対は 3 ラウンドで拡張: **ADR 28→43→48→51** (round-1: cross-doc 平文/タグ併記・swap・削除・attr-vs-visible。 round-2: nested-tag early-match /
  hyphen-tag swap / dec-kick / prin-id dup-decoy。 round-3: cover-meta / cxid / drid)・**research 49→52→54** (wrapper-tag swap + nested-tag / hyphen-tag)・
  **SRS 40→42→43** (偽 機械SSoT / 偽 検証状態 / `</b>` 外平文追記の block-scoped 捕捉)。 全 fixture verify PASS・validate clean・sandbox 37/37。
