...
- **chrome (folio:chrome) 共存**: head JSON-LD を持つ生成 page は `folio_chrome_targets` の一員になり、 `folio build` の chrome 機構 (`folio_chrome_render_doc`・ADR-0039 §2.3) が breadcrumb/skip-link/TOC を `<body>` 直後・最初の `</header>` 直後・`</body>` 直前へ注入・維持する。 deck 帯 (`doc-cover-band`) は body 提示層としてそのまま **共存** し、 chrome (移動する機構の層) に置換されない (grill F7 裁定)。 marker は build 注入で carry される (assembler は head JSON-LD で参加資格を与え、 chrome 実体は corpus 依存ゆえ build が所有)。
  - **例外: allowlist 掲載 pack の産物は chrome-less (ADR-0054 §2.1/§2.4・folio-2s60 で land・folio-qojv 決定 9 で 2 本へ拡張)**。 `<meta name="generator" content="folio spec-pack assembler…">` / `<meta name="generator" content="folio principle-pack assembler…">` を持つ生成 page (spec-pack = spec / relations / verification / srs-verification fork、 principle-pack = constitution) では、 build は **breadcrumb と TOC を注入せず audience-toggle 生成 JS も出さない** — 提示層は pack が完結所有する。 存続するのは skip-link (focus 時のみ可視) / print 全展開 handler / prev-next で、 加えて所属 locator (`p.doc-locator` =「この文書の所属 … 一覧へ」) を prev-next 直上へ注入する。 判定述語は `folio_chrome_is_generated_spec` に単一定義 (新 marker を発明しない) で、 対象 pack 集合は `folio_chrome_pack_alt` の **単一 table** に持つ (canonical 述語と家系述語がともに同 table から導出される = 片方だけ広げる退行を構造的に封じる)。 **table 外の pack (ADR-pack / vision-pack / S4 SRS …) は従来どおり chrome 有り** — ADR-0054 は allowlist 外の pack 産物を chrome-less 化していない。 vision-pack は「将来 vision doc が `spec/` へ入った瞬間に無審査で chrome を剥がす休眠 fail-open」を避けるため、 arch-pack は folio-8kuf の不適合確定により、 いずれも **意図的に不掲載**。 二方向の enforcement は `folio validate` の chrome-scope arm (nav-regen-drift 内・生成 cohort / 非生成 cohort の対称 assert + 誤分類 anchor + 逆向き anchor) と sandbox scenario `chrome-scope-{clean,violation}` (spec-pack) / `chrome-scope-principle-{clean,violation}` (principle-pack)。 chrome 系 CSS は `common.css` に在るが生成 page は common.css を読まない (pack の inline `<style>` が提示層 SSoT) ため、 locator / skip-link の見た目は pack 側 emit 改修 (folio-31oz の Leg 1) で追随させる。
