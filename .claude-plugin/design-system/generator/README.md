...
- **core 昇格 (`verify_cross_doc_refs`)**: B3 まで verify-adr.sh §3 と verify-research.sh §3 に *同型の cross-doc 解決
  ブロックが重複* (= 3 度目の重複)。 doc-type 非依存の **スケルトン 8 検査** (照会先 contract 実在 / doc_id 一致 /
...
- **★可視 echo 厳密一致の ADR 横展開 (Part 2b)**: research の round-2/4 ceiling 結晶 (全 `<b>` 列挙 + 全タグ除去後の
  可視テキスト == 固定テンプレ) を ADR の 3 可視 echo へ適用 — 表紙 ref-chip (`<b>` ちょうど 2 本 = srs_doc_id, join(req,・)) /
  justify-tgt (`<b>` 無し平文の照会先) / justify-row (可視 req == data-justifies-req の attr-vs-visible)。 各ブロックは
  count anchor で個数も固定 (ブロックごと削除 = while 空回りの fail-open を封鎖)。 タグ併記・平文併記・swap・別タグ注入・
  第2 `<b>` 注入を一括封鎖する *不動点* (echo は固定テンプレ=自由文なし)。
- **SRS 監査 = 該当 fail-open 無し (clean)**: SRS-pack は前方 cross-doc 構造を持たない (照会の終端)。 監査した 3 クラス —
  (a) `comm -23` 空値畳み込み = **不在** (backward/acceptance は `set_eq` 文字列等価 + count anchor で、 comm は診断出力のみ)・
  (b) 完全決定的ブロックの可視 echo の緩い照合 = gate D `fid == data-req-id` は既に厳密 attr-vs-visible (全列挙)・
  gate H footer は timestamp 非決定的ゆえ「非空のみ」が設計上正当 (cross-doc echo ではない)・rtm-summary 可視は S7 で
  overclaim 撤回済 (意図的 ceiling 領域)・(c) sample-not-enumeration = **不在** (`head -1` 依存なし・全 gate は count か `/g` 列挙)。
  → **SRS に無い cross-doc 構造を捏造せず**、 自由文内容 fidelity は ceiling 領域ゆえ floor で追わない (two-gate 境界厳守)。
- **非破壊**: 生成 artifact 全 8 種 byte 不変 (footer timestamp 除く)・research 敵対 49/49 緑のまま (= 昇格でスケルトン非弱化の
  一次証拠)・ADR 敵対 28→**38** (cross-doc クラス 10 ケース追加: 空 req abort / ref-chip 平文併記・第2`<b>`・別タグ・swap・削除 /
  justify-tgt 改竄・削除 / justify-row attr-vs-visible / justify-req span 削除)。
