...
- **非破壊**: 生成 artifact 全 8 種 byte 不変 (footer timestamp 除く・assemble は無改変ゆえ verify 側のみの変更)・昇格でスケルトン非弱化。
  敵対は 2 ラウンドで拡張: **ADR 28→43→48** (round-1: 空 req abort / 平文併記・第2`<b>`・別タグ・b-swap・削除 / jh 改竄・swap・削除 / attr-vs-visible /
  wrapper-tag swap。 round-2: nested-tag early-match / hyphen-tag swap / dec-kick 改竄 / prin-id duplicate-decoy)・**research 49→52→54** (wrapper-tag swap +
  nested-tag / hyphen-tag)・**SRS 40→42→43** (偽 機械SSoT / 偽 検証状態 / `</b>` 外平文追記の block-scoped 捕捉)。 全 fixture verify PASS・validate clean・sandbox 37/37。
