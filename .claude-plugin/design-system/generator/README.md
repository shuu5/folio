...
```
contract.yaml ──validate──▶ assemble.sh ──▶ HTML (内容=決定的・prose スロット空) ──③──▶ [opus prose 充填] ──▶ 完成
              (fail-closed)  (決定的・捏造不可)        ▲                                  (後段スライス)
                                                       ├ verify-fabrication-free.sh で機械証明
...
- **③ opus prose 充填** (後段スライス) — 空 `data-prose-slot` だけを埋める。 ADR-0042 §2.1 の opus スロット =
  章リード / plain (やさしい言い換え) / **「なぜ要る」根拠** / 1 文サマリ。
...
```bash
./assemble.sh contract/ec-checkout.srs.yaml out.html          # 生成 (内容=決定的、 prose スロット空)
...
- **prose 空** — 章リード/plain/根拠/サマリは opus 待ち (③)。 example は充填済。
...
- **S5 floor gate**: `data-prose-slot` 非空 (no-TBD) を本 verify と同じ前提 (`<style>` 除去 + perl 要素単位の空判定) で実装。

## 重要 gotcha

- **bash 5.2+ patsub_replacement**: `${v//pat/repl}` の repl 中の生 `&` が「マッチしたテキスト」後方参照になり、
  HTML escape を破壊する (`<` → `<lt;`)。 assemble.sh は冒頭で `shopt -u patsub_replacement` し無効化する。
...
ceiling review 3 ラウンド反映: wf_41fcbde3 (第一スライス: escape 追加 / 改行 phantom / acceptance 無検証 等)、 wf_82c7b956 (リッチ化:
**esc の patsub_replacement 破綻** = 第一スライスで追加した escape が bash 5.2 で無効だった blocker / A1 の否定検証 false-pass /
nfr.hero null 漏れ / RTM 行ラベル欠落 / cover ID レンジ / actor.tint CSS allowlist) を全反映。
