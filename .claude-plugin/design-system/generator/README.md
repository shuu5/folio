...
- **★ds8 ceiling (admin 独立検証で発見・修正した 4 major fail-open)**:
  ...
  - **wrapper-tag swap (marker-keyed parity)**: 可視 echo の while-regex が tag 固定 (`<div>`/`<p>`) で count anchor は marker-only =
    ...
    B3 の「可視テキスト厳密一致=不動点」が wrapper-tag 選択で兄弟経路を残していた)。 while を **marker-keyed** (`<(\w+)\b … marker …>(.*?)</\1>` =
    マーカーを担持する任意タグを捕捉) にして marker-only count とパリティを取り、 ADR + research の全 echo で封鎖した。
  - **gate H の緩い照合**: verify-srs gate H が決定的 2 フィールド (機械SSoT = contract basename / 検証状態 = 固定2状態) を「非空のみ」で
    照合 = 偽 provenance や偽『GREEN 認定済』検証状態が floor を素通る fail-open。 機械SSoT は `${CONTRACT##*/}` と・検証状態は固定2状態と
    厳密一致を要求 (生成=timestamp のみ非空)。 → 当初の「SRS 監査 clean」は gate H について overclaim だった (是正済)。
...
- **次の rule-of-three watch**: 可視 echo の micro-pattern (block 抽出 / `<b>` 全列挙 / 本数 assert / visible==template) は research 4 + ADR 4 で
  反復するが、 binding 変動が高く (per-block attr vs 固定値・`<b>` 本数 0/1/2) variadic 抽象は過剰抽象リスクが高いため **抽出は強制せず** 別 doc-type の
  4 例目出現時に再評価する (B3 rule-of-three 監視の継続)。
- **非破壊**: 生成 artifact 全 8 種 byte 不変 (footer timestamp 除く)・research 敵対 49→**52** 緑 (swap ケース追加・昇格でスケルトン非弱化の
  一次証拠)・ADR 敵対 28→**43** (空 req abort / ref-chip 平文併記・第2`<b>`・別タグ・b-swap・削除 / jh 改竄・swap・削除 / ref-chip・justify-tgt
  wrapper-tag swap / justify-row attr-vs-visible / justify-req span 削除)・SRS 敵対 40→**42** (gate H 厳密一致: 偽 機械SSoT・偽 検証状態)。
