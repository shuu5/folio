...
- **`verify-fabrication-free.sh` §7f marker 占有数パリティ (round-3/4 = ★round-2/3 ceiling 反映)**: round-2 ceiling (wf_997ee765) が §7f(h) の
  ...
  ds8 不動点 *marker-only count parity* で封鎖し、 round-3 ceiling 反映で堅牢化: (a) ★**quote 構文非依存の token-match** (core `count_attr_token`) —
  assembler は double-quote のみ emit ゆえ single-quote (`class='fid'`)・unquoted (`class=fid`)・multi-class (`class="x fid"`) は全て tamper だが素朴な
  `grep 'class="fid"'` を素通る → 属性値を空白分割しトークン完全一致で数える (HTML 属性構文の揺れに堅牢な不動点)、 (b) ★**統制値も global+row-scope 二重** —
...
- **非破壊**: assemble/inject/css 無改変 → 生成 artifact byte-identical (floor 強化は verify 側のみ)。 敵対 **SRS 44→55→66→69→78** (round-1 A36-A46 + round-2 A47-A57 +
  round-3 A58-A60 [decoy/ghost fid/ghost nid] + round-4 A61-A67 [single-quote ghost fid/nid・chrome ghost prio/vmeth・rtm 行見出し id・受入ドット可視・delete-legend+add-row・unquoted/multi-class ghost])・**ADR 52→53** (A51: drg)。
...
./test-adversarial.sh                                                            # 78 ケース: assembler + prose + term-inline + verify-srs floor + gate F selftest + ds8 gate H/cover-meta + dty 識別子/構造値 (§7e+§7f+marker count) の回帰
```
