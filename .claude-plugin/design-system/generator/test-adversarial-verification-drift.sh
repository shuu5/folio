#!/usr/bin/env bash
# test-adversarial-verification-drift.sh — folio-lwhz F8 drift gate (verify-verification-drift.sh)
...
GATE="$SCRIPT_DIR/verify-verification-drift.sh"
...
echo "== verification drift gate 敵対回帰 (F8・folio-lwhz) =="

# --- H0 precision: healthy canonical → exit 0 + PASS 行 (clean corpus を誤検出しない) ---
out="$("$GATE" 2>&1)"; rc=$?
if [[ "$rc" -eq 0 ]] && grep -qF 'PASS: design-intent/spec/verification.html == fresh 再生成' <<<"$out"; then
...
echo "  ----"
printf '  verification-drift 敵対: %d/%d PASS\n' "$pass" "$total"
