...
echo
echo "--- template-ban: SRS HTML は <template> ゼロ (FF5 declarative shadow DOM census-blindness を静的封鎖・folio-hef S1) ---"
...
chk "template-ban: <template> 出現 == 0 (FF5 declarative shadow DOM 静的封鎖・whole-tag ban)" 0 "$shadowN"

echo
echo "--- nested-context-ban: SRS HTML は nested browsing context ゼロ (FF5-sibling census-blindness を静的封鎖・folio-hef S1 ceiling round-2) ---"
...
chk "nested-context-ban: <iframe|object|embed|frame|frameset|portal> 出現 == 0 (FF5-sibling census-blindness 静的封鎖)" 0 "$nbcN"

echo
echo "--- inline-only-ban: SRS HTML は inline-HTML + geometric SVG 以外の hidden-render 機構ゼロ (FF5 census/fidelity 両盲点を doc-type 不変条件で封鎖・folio-hef S1 ceiling round-3・user 案A) ---"
...
chk "inline-only-ban: <img|image|canvas|video|audio|input|textarea|select|button|progress|meter> 出現 == 0 (FF5 hidden-render 封鎖)" 0 "$hrenN"
...
chk "inline-only-ban: url(#fragment 以外) 出現 == 0 (FF5 CSS resource sink hidden-render 封鎖・char-ref robust)" 0 "$urlN"

echo
echo "--- scroll-pseudo-ban: generated-content scroll pseudo ゼロ (::scroll-button / ::scroll-marker / scroll-marker-group・round-3c carve agent 実証) ---"
...
chk "scroll-pseudo-ban: scroll-button/scroll-marker(-group) 出現 == 0 (generated-content pseudo 捏造の静的 backstop)" 0 "$spseudoN"

echo
echo "--- list-marker-ban: generated marker テキスト (counter-style / 文字列 list-style) ゼロ (round-3d ceiling・list-marker 新次元を S1 で静的封鎖) ---"
...
chk "list-marker-ban: @counter-style / 文字列 list-style(-type) 出現 == 0 (generated marker 捏造の静的封鎖)" 0 "$lmN"
...
echo
echo "--- bidi-override-ban: <bdo> / CSS unicode-bidi override ゼロ (round-3e ceiling・制御 codepoint 無しの視覚反転を封鎖) ---"
...
chk "bidi-override-ban: <bdo> / unicode-bidi:(bidi|isolate)-override 出現 == 0 (制御 codepoint 無し視覚反転封鎖)" 0 "$bdoN"
