...
# usage: assemble-srs.sh <contract.yaml> [out.html]
#
# B2 (folio-5ua): 共通 idiom (q/esc/mark_terms/band/cover骨格/glossary/footer/finalize) は
# lib/common.sh (core) から source する。 本 file は SRS-pack 固有の section emitter を残す。
...
CONTRACT="${1:?usage: assemble-srs.sh <contract.yaml> [out.html]}"
OUT="${2:-/dev/stdout}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CSS="$SCRIPT_DIR/../srs.css"
[[ -f "$CONTRACT" ]] || { echo "assemble-srs: contract not found: $CONTRACT" >&2; exit 1; }
[[ -f "$CSS" ]] || { echo "assemble-srs: srs.css not found: $CSS" >&2; exit 1; }
command -v yq >/dev/null || { echo "assemble-srs: yq required" >&2; exit 1; }
...
# ---- icon SVG (SRS-pack 固有。 共用 icon=ICO_FLOW/SHIELD/BOOK/CHECK_BIG/USER + ico() は lib/common.sh) ----
...
# ---- fail-closed contract validation (普遍規律 = core_validate_*、 SRS 固有 = trace/EARS/priority/tint) ----
validate() {
  local errs=0 d p
  core_validate_strings "assemble-srs" || errs=1
  d="$(q '(.requirements[].id, .nfr[].id)' | sort | uniq -d)"; [[ -z "$d" ]] || { echo "assemble-srs: 要件/NFR id 重複: $d" >&2; errs=1; }
  d="$(q '.upper_needs[].id' | sort | uniq -d)";               [[ -z "$d" ]] || { echo "assemble-srs: ニーズ id 重複: $d" >&2; errs=1; }
  d="$(q '.acceptance[].id' | sort | uniq -d)";                [[ -z "$d" ]] || { echo "assemble-srs: 受入 id 重複: $d" >&2; errs=1; }
  d="$(comm -23 <(q '(.requirements + .nfr)[].trace.backward[]' | sort -u) <(q '.upper_needs[].id' | sort -u))"
  [[ -z "$d" ]] || { echo "assemble-srs: 未定義の上位ニーズ参照: $d" >&2; errs=1; }
  d="$(comm -23 <(q '(.requirements + .nfr)[].trace.acceptance[]' | sort -u) <(q '.acceptance[].id' | sort -u))"
  [[ -z "$d" ]] || { echo "assemble-srs: 未定義の受入基準参照: $d" >&2; errs=1; }
  d="$(comm -23 <(q '.acceptance[].links[]' | sort -u) <(q '(.requirements[].id, .nfr[].id)' | sort -u))"
  [[ -z "$d" ]] || { echo "assemble-srs: 受入の links が未定義の要件を指す: $d" >&2; errs=1; }
  for p in $(q '.requirements[].ears.pattern'); do [[ -v EARS_CLASS[$p] ]] || { echo "assemble-srs: 未知の EARS pattern: $p" >&2; errs=1; }; done
  for p in $(q '.requirements[].priority');     do [[ -v PRIO_LABEL[$p] ]] || { echo "assemble-srs: 未知の priority: $p" >&2; errs=1; }; done
  for p in $(q '.actors[].tint'); do case "$p" in brand|violet|warn|info|ok|bad) ;; *) echo "assemble-srs: 未知の actor tint (CSS allowlist 外): $p" >&2; errs=1 ;; esac; done
  core_validate_glossary_substring "assemble-srs" || errs=1
  [[ "$errs" -eq 0 ]] || { echo "assemble-srs: contract validation FAILED (fail-closed)" >&2; exit 1; }
}
...
emit_cover() {
  core_emit_cover_head "この文書が約束すること (1 文サマリ)"
...
# footer は core_emit_footer に SRS-pack 別のタグ列を渡す (本文 SSoT 行は共通)。
emit_footer() {
  core_emit_footer '<span>folio design system</span><span>deck × B2</span><span>ISO/IEC/IEEE 29148</span><span>S4 生成</span>'
}
