...
[SRS 部品 taxonomy](../architecture/research/srs-component-taxonomy.html) §5.3 が定義する ceiling (意味判定) を completeness 軸へ拡張する一翼 (§5.2.6 集合完全性の ceiling 昇格)。 生成 SRS プレゼンの完全性判定は **floor (機械) + ceiling (意味) の二層**で、 `GREEN ⟺ (floor 全通過) AND (ceiling 合格)`。 ceiling は複数 lens の束で、 既存の 2 翼は [persona-walk-srs](persona-walk-srs.md) (gate I = 読みやすさ) と [fidelity-srs](fidelity-srs.md) (gate J = 捏造/歪み)。 本 agent は**第 3 の ceiling lens = 意味的完全性 (completeness)** を担い、 他 2 翼と領分が重ならない (§5)。 **ただし現行 SSoT の ceiling は 2 翼 (persona + fidelity) のまま**であり、 本 lens を co-equal な第 3 翼として GREEN 判定に組み込むのは、 taxonomy §5.3 / `verify-srs.sh` L24 の **2→3 翼 amendment 後**に有効化される予定 (それまで本定義は skeleton・未 gating、 §deferred の有効化前提を参照)。
...
### 機械可読 block (凍結・adjudicate が数える)

本文末尾に、 orchestrator の adjudicate が集計するための機械可読 block を必ず出す (severity ラベルは本 agent = LLM が付ける = 境界の LLM 側。 機械はこれを**数えるだけ**):
...
★ **有効化前提 (gating dependency)**: 現行 SSoT — taxonomy §5.1 (判定式) / §5.3 (gate 集合) と `verify-srs.sh` L24 — は ceiling を **2 翼 (persona + fidelity)** と規定する。 本 lens を co-equal な第 3 翼として GREEN 判定に組み込むには、 **taxonomy §5.1/§5.3 と `verify-srs.sh` L24 の 2→3 翼 amendment** が前提 (`folio ceiling-anchors` CLI / funnel 実装**だけでなく**、 floor script + taxonomy の gating 拡張に依存する)。 amendment 前は本定義は **skeleton (未 gating)** で、 GREEN 判定には参加しない。
...
- [SRS 部品 taxonomy](../architecture/research/srs-component-taxonomy.html) §5.1 (判定式 `GREEN ⟺ floor AND ceiling`) / §5.2 gate A・G (存在の floor) / §5.3 gate I・J (ceiling の既存 2 翼) / §5.2.6 (集合の completeness/consistency)
...
- floor: `.claude-plugin/design-system/generator/verify-srs.sh` (gate A-H の決定的検査) — floor 通過は `CEILING=PENDING`、 ceiling の合格で初めて GREEN。 **現行 SSoT の ceiling は 2 翼 (persona + fidelity)** = `verify-srs.sh` L24 / taxonomy §5.3 gate I・J。 completeness を加えた **3 翼への拡張は amendment 後** (上記 deferred の有効化前提)
