...
> **★kicker (§N / トピック ラベル) の暫定検査 (floor 未被覆・follow-up folio-l93)**: 各 band の `kicker` (`§N / トピック`) は contract の `sections[].kicker` 由来の決定的フィールドだが、 現状 floor (`verify-spec.sh`) は heading/essence 列しか突合せず **kicker を検査しない**。 floor 昇格 (folio-l93・doctrine = 決定的→floor) までの**暫定**として、 gate J は drift の一部で **`kicker` のトピック語が当該 section の `heading`/`essence` と別章のトピックを指していないか** (topic swap・取り違え) を意味検査する。 純 `§N` 番号の決定的整合 (heading の `§N.` との一致) は reader-map として gate I = [persona-walk-spec](persona-walk-spec.md) が拾う。 floor 昇格後は本暫定項を撤去し、 kicker は floor 被覆事項として §5 へ移す。
...
- **構造の集合一致 / 機械可読 key / 決定的フィールド値 / 照会 chip 集合一致 は floor の担当** — 件数一致・id 一意性・doc_type==rules・可視 heading/essence 順序・**要件タプル (id・ears_pattern・badge class/label・essence・statement) の決定的可視テキスト厳密一致**・block 可視テキスト順序・照会 chip の token/doc/role echo (count・SET・role allowlist・(token,role) ペア・可視 `<b>` == attr)・cover-meta 集計・core 共通 chrome・escape 健全・no-TBD・注入忠実 (`--filled`)。 本 agent は**再検査しない** (気付いても low で「floor 被覆」と言及するに留める)。 本 agent の領分は**意味的 fidelity** (prose が SSoT を忠実に要約するか・chapter-lead/cover-summary が section essence に接地し規約作文しないか・EARS pattern が statement 論理と一致するか・非終端性を保つか・前方照会が意味的に妥当か) に集中する。 (★例外 = band の `kicker` は現状 **floor 非被覆** ゆえ、 §2(a) 末尾の暫定項どおり kicker のトピック drift を gate J が拾う。 follow-up [folio-l93] で floor へ昇格後は撤去。)
