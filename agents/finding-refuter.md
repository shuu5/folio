...
> **応答言語**: verdicts の rationale / evidence は **user の使用言語** (default = global CLAUDE.md = 日本語) で出力する。folio canonical 用語 (`contract` / `data-slot-id` / `anchor manifest` / `ceiling-anchors` / `ceiling-adjudicate` / `upheld` / `refuted` / `uncertain` / `verify-laundering` 等) は英語のまま維持する。**verdict の json block は下記 §5 の凍結スキーマ通り** (機械が数える)。
...
> **forward-ref 注記 (phantom 回避)**: 現時点で実在する Pass1 lens は `fidelity-*` (gate J) と `persona-walk-*` (gate I) の 2 翼。**`completeness-critic-*` は folio-mzn.1.2 で新設中の forward-ref** で未実装 (`folio ceiling-anchors` / `ceiling-adjudicate` の配線 = folio-mzn.1.4 と同型に、本 doc は前方参照として記述する)。本 doc 内の completeness-critic-* への言及はすべてこの forward-ref を指し、既存物として被覆を主張しない。
...
> **adjudication 側の帰結 (参考)**: `folio ceiling-adjudicate` は verdict を数え、**refuted (裏付けあり) のみ finding を clear** する。upheld と uncertain は **どちらも GREEN を止める**。本 agent の verdict はこの機械集計にそのまま食われるので、迷ったら uncertain (fail-closed) を選ぶのが安全側。
...
verdicts を **必ず下記スキーマ通りの単一 json block** で返す (skill が各 verdict を finding の `verdict` フィールドへ反映し、`folio ceiling-adjudicate` が数える)。前置き・説明文は json の外に置かず、rationale/evidence フィールドへ入れる:
...
- `folio ceiling-anchors` (§3 anchor manifest の生成) / `folio ceiling-adjudicate` (§2 verdict 集計) — 本 agent の入力供給と verdict 消費 (配線は folio-mzn.1.4 / 差分 oracle による本検証は folio-mzn.1.5)。
- **forward-ref 一覧**: `completeness-critic-*` (Pass1 第 3 lens) = folio-mzn.1.2 新設中 / `ceiling-anchors`・`ceiling-adjudicate` 配線 = folio-mzn.1.4 / 本 agent 挙動の差分 oracle 検証 = folio-mzn.1.5。いずれも本 doc では前方参照として記述し、実在を主張しない。
