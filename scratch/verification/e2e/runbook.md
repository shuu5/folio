...
`scratch/specs/` 配下の Edit/Write は caller-marker hook で gate される。次のどちらかで allow:
...
2. `scratch/specs/e2e-probe.html` を Write。content は object 形式 JSON-LD
...
**後始末**: `rm -f .folio/architect-active` → `rm -f scratch/specs/e2e-probe.html`
...
**操作**: marker UNSET (`rm -f .folio/architect-active`) の状態で `scratch/specs/e2e-b1.html` を Write。

**期待観察**: file **不在** (caller-marker が PreToolUse deny)。← live hook の決定的確認点。

### B2: spec_path 外で spec を作成 → deny

**操作**: marker SET で `scratch/random-e2e/e2e-b2.html` を Write。
...
**操作**: marker SET で `scratch/specs/e2e-b3.html` を Write。
...
**操作**: marker SET で `scratch/specs/e2e-b4.html` を Write。
...
**S-B 後始末**: `rm -f .folio/architect-active scratch/specs/e2e-b3.html scratch/specs/e2e-b4.html`
+ `rm -rf scratch/random-e2e/`
...
**操作** (marker UNSET):
1. C1: `scratch/random-e2e/e2e-c1.html` を Write (plain HTML、folio-doc-type=spec **無し**)。
...
**後始末**: `rm -rf scratch/random-e2e/` + `git checkout -- README.md`
...
- E1 (unset 復帰確認): S-A/S-B の marker SET 群の後、`rm -f .folio/architect-active` してから
  `scratch/specs/e2e-e.html` を Write → file **不在** (deny 復帰)。marker 残留による fail-open が
...
- [ ] probe file を全削除した (`scratch/specs/e2e-*.html` / `scratch/random-e2e/`、README.md 復元)
