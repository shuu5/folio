...
folio spec は HTML を表現基体としており、 plain text を超える **visual richness** を活用できる (canonical list = [rules.html §4.5 Visual Richness Patterns](../../../architecture/spec/rules.html#s4-5-visual))。 grill 中、 settle した domain の構造を spec に materialize する際は、 architect が domain pattern に対応する visual element 候補を **user に推奨答えとして提示**してから採用する (silently choosing 禁止、 P-8)。

### grill 中の声かけ例

- **状態 lifecycle** が settle した時 → 「overview §X に mermaid `stateDiagram-v2` で live / tombstone / expired / 未存在 の遷移図を追加するのが推奨です。 採用しますか？」
- **caller-system 対話 sequence** が settle した時 → 「`sequenceDiagram` で API client→resolver の HTTP やりとりを可視化しますか？」
- **decision 分岐** (ADR alternatives) → 「`flowchart` または `<table>` で採用/不採用 candidates × criteria を対比しますか？」
- **REQ-ID 多数 × constitution 原則 trace** が必要になった時 → 「`<table>` で traceability matrix を作りますか？」
- **long enumeration** (optional / advanced 内容) → 「`<details>` + `<summary>` で折りたたみますか？」

### 留意

- visual richness は **domain 構造を反映する手段**で装飾ではない (P-3 WHAT-only 整合)。 状態を持たない domain に stateDiagram は不要 — 「使わない」も正しい設計判断。
