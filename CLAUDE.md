...
├── agents/                                 review subagent (公式仕様 = plugin root 直下): Phase F spec-review-* 5 本 + readability-walk (rules §11.4 readability lens) + doc-type 別 ceiling 対 11 doc-type (persona-walk-{srs,adr,research,principle,spec,vision,glossary,architecture,testcases,datamodel,interface} = gate I / fidelity-同 11 種 = gate J、 taxonomy §5.3) + completeness-critic-srs (§5.2.6 第 3 lens 候補) + finding-refuter (ceiling 2-pass 汎用・中立 bias)
