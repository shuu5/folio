...
**ADR-0054 (2026-07-22)**: 生成 spec ページ (flip 済 canonical) は chrome-less — folio build は nav.toc / breadcrumb を注入せず audience-toggle も行わない (skip-link は focus 時のみ可視・prev-next は下部導線として存続)。提示層は pack assembler が完結所有。ADR-0039 の chrome 注入は生成 spec について適用範囲 narrowing (frozen 本文不触・本注記が trace)・rules.html §11 の「全ページ chrome」MUST は生成 spec について supersede (spec 本文の整合改訂は folio-31oz で contract 経由)。**chrome 側 (Leg 2) は folio-2s60 で land 済** — 検出述語は既存の `<meta name="generator" content="folio spec-pack assembler…">` (新 marker 不発明・共通 helper `folio_chrome_is_generated_spec` で単一定義)、生成 spec には所属 locator (`p.doc-locator`) を prev-next 直上へ注入、二方向 assert は `folio validate` の chrome-scope arm (nav-regen-drift 内) + sandbox scenario `chrome-scope-{clean,violation}`。過渡期に残るのは pack emit 側 (Leg 1/3/4/5 = 各 spec cell)。**述語の pack allowlist は folio-ni9i.2 で 2 本へ拡張済 (folio-qojv 決定 9 = 案 P)** — 対象 pack 集合は `folio_chrome_pack_alt` の単一 table (spec-pack + principle-pack) に持ち、canonical 述語と家系述語がともに同 table から導出される。principle-pack は §2.4 の constitution flip (folio-8dkl) 前提として先行掲載、vision-pack (休眠 fail-open 回避) と arch-pack (folio-8kuf で不適合確定) は意図的不掲載。principle-pack 側の二方向 pin は sandbox scenario `chrome-scope-principle-{clean,violation}`。REQ-NAV-008 の条文追随は folio-altm (ni9i.3) 所有。
