...
Claude Code が session 開始時に自動 read する project-level instruction。 本 file の位置づけは `constitution.html` P-11 (HOW を spec に書かない) の **root 例外** として platform binding (HOW) を許容する場所 (folio-self-spec.html §9 Bindings)。
...
2. `constitution.html` — 12 不変原則 3-tier (Always 7 / Ask first 3 / Never 2)、 2 mermaid 図 (§3 §4 cross-ref スタブ)
3. `rules.html` — Layer 1 consumer 向け universal rules (markup + naming + §10 Mandatory Actions)
4. `folio-self-spec.html` — folio Layer 0 framework 自身の architecture spec (harness + plugin integration + bindings、 8 mermaid 図)
5. `decisions/ADR-0001-architecture-rules-split.html` — ADR-0001 (architecture-rules split decision、 v3 accepted)

上記 5 file を読まずに spec edit を行ってはならない。
...
これは rules.html §10.1 (MUST 規範 REQ-CM-001〜003) + folio-self-spec.html §7.3 (hook flow chart) で規定される caller marker 機構の **Claude Code 固有 binding 実装** である (platform-agnostic な WHAT に対する、 本書の HOW binding)。 spec 編集を行う tool (Edit/Write/NotebookEdit) を呼ぶ前に必須。
...
## 3. 3 axis での区別 (LLM 混同回避 5 層防御、 folio-self-spec.html §7.4)
...
5 層全てを通過したときのみ folio repo の spec を編集できる (folio-self-spec.html §7.4 参照)。
...
- spec / ADR / constitution / rules / folio-self-spec / changes proposal: HTML
...
## 6. plugin 構造 (folio-self-spec.html §8)
...
- Claude Code 公式 convention に従い repo root 維持。 将来 platform 拡張時は `core/bindings/claude-code/` への移管も検討 (folio-self-spec.html §9 Bindings)。
